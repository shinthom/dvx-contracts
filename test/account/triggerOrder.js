...
describe("triggerOrder", () => {
  it("long", async () => {
    ...
    {
      await setPrice(WETH, acceptablePrice - 1n, acceptablePrice - 1n);
      await expect(
        warehouse
          .connect(orderKeeper)
          .executeTriggerOrder(account.target, positionKey, 1, {
    ...
    await setPrice(WETH, acceptablePrice, acceptablePrice);
    await warehouse
      .connect(orderKeeper)
      .executeTriggerOrder(account.target, positionKey, 1, {
        value: minExecutionFee,
  ...
  it("short", async () => {
    ...
    const triggerOrderType = { takeProfit: 0, stopLoss: 1 };
    const triggerPrice = ethers.parseUnits("2000", 30);
    const acceptablePrice = ethers.parseUnits("2200", 30); // calculated by slippage tolerance

    const ethPrice = ethers.parseUnits("2000", 30);
    ...
    {
      const notAcceptablePrice = ethers.parseUnits("1800", 30); // calculated by slippage tolerance
      await expect(
        account
          .connect(user)
          .createTriggerOrder(
    ...
    await warehouse.setPriceMinDeviation(100); // 1%
    {
      const notAcceptablePrice = ethers.parseUnits("2019", 30); // calculated by slippage tolerance
      await expect(
        account
          .connect(user)
          .createTriggerOrder(
    ...
    await faucet(collateral, collateralAmount);
    ...
    // todo: ProviderError: execution reverted: SafeMath: subtraction overflow
    // {
    //   await setPrice(WETH, acceptablePrice + 1n, acceptablePrice + 1n);
    //   await expect(
    //     warehouse

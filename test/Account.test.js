...
describe("Account", async () => {
  ...
  describe("limit order", () => {
    it("scenario", async () => {
      const { account, warehouse } = await loadFixture(deploy);
      ...
      await account.registerLimitOrder(limitOrder);
      expect(await warehouse.totalLimitOrders()).to.be.equal(1);

      const registeredLimitOrder = (await warehouse.getLimitOrder(0))[2];
      const gmxOrder = {
        collateral: registeredLimitOrder.collateral,
        index: registeredLimitOrder.index,
        collateralAmount: registeredLimitOrder.collateralAmount,
        leverage: registeredLimitOrder.leverage,
        isLong: registeredLimitOrder.isLong,
        collateralPrice: 0,
        indexPrice: 0,
      };
      const muxOrder = {
      ...
      // todo: createLimitOrder after quoter is implemented.
    });
  });
  ...
  describe("trigger order", () => {
    ...
    it("scenario", async () => {
      const { account, gmxV1, weth, warehouse, executeIncreasePosition } =
        await loadFixture(deploy);
      ...
      await account.registerTriggerOrder(triggerOrder);
      ...
      // todo: fix error
      // await warehouse.createTriggerOrder(account.target, 0);
    });
  });
});

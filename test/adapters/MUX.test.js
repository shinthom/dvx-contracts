...
describe("MUX", () => {
  ...
  describe("pnl", () => {
    it("usdc -> eth: long, increasePosition", async () => {
      const {
        mux,
        user,
        account,
      ...
      const order = await mux.makePositionOrder(USDC, WETH, collateralAmount, leverage, isLong, usdcPrice, wethPrice); // prettier-ignore
      ...
      {
        await checkBalance(account);
        console.log("position: ", await mux.getPosition(account.target, USDC, WETH, isLong)); // prettier-ignore
      }
      var price = ethers.parseUnits("2200", 8); // +10%
      await replaceOracleReferenceAndSetPrice(WETH, price);
      await account.connect(user).createMarketOrders([mux.target], [{ orderType: order.orderType, path: [...order.path], index: order.index, collateralAmount: order.collateralAmount, size: order.size, isLong: order.isLong }]); // prettier-ignore
      await fillPositionOrder();
      {
